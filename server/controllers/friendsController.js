  ...
  getAll: async (req, res, next) => {
    ...
    try {
      res.locals.friends = await new Promise((resolve, reject) => {
        db.all(
          'SELECT u2.first_name, u2.last_name, u2.latitude, u2.longitude FROM friends JOIN users u1 ON u1.id = friends.user_id1 JOIN users u2 ON u2.id = friends.user_id2 WHERE u1.first_name = ? AND u1.last_name = ?',
          [firstName, lastName],
          (err, rows) => {
            if (err) {
              reject(err);
